...
class ExchangeNotInCollectionsError(Exception):
    """Exchange not in Collection Error."""

    def __init__(self, market_ticker):
        """Initialize."""
        super(ExchangeNotInCollectionsError, self).__init__(
            "{} is either an invalid exchange or has a broken API.".format(
                market_ticker
            )
        )
...
def print_profit_opportunity_for_path(
    ...
    """Print profit opportunity for graph."""
    if not path:
        return
...
def print_profit_opportunity_for_path_multi(
    graph: nx.Graph, path, print_output=True, round_to=None, shorten=False
):
    """Print profit opportunity for path multi.
    ...
    for i in range(len(path)):
        if i + 1 < len(path):
            ...
            if round_to is None:
                ...
                result += "{} to {} at {} = {}".format(
                    start, end, round(rate, round_to), round(money, round_to)
                )
            if not shorten:
                result += " on {} for {}".format(
                    graph[start][end]["exchange_name"], graph[start][end]["market_name"]
                )
