...
def create_weighted_multi_exchange_digraph(
    exchanges: list, name=True, log=False, fees=False, suppress=None
):
    """Create weighted multi exchange graph.
    ...
    futures = [
        asyncio.ensure_future(exchange_dict["object"].load_markets())
        ...
        for exchange_dict in exchanges:
            if "maker" in exchange_dict["object"].fees["trading"]:
                ...
                if "fees" not in suppress:
                    warnings.warn(
                        "The fees for {} have not yet been implemented into the library. "
                        "Values will be calculated using a 0.2% maker fee.".format(
                            exchange_dict["object"].id
                        )
                    )
    ...
    graph = nx.MultiDiGraph()
    futures = [
        _add_exchange_to_multi_digraph(graph, exchange, log=log, suppress=suppress)
        for exchange in exchanges
    ]
    loop.run_until_complete(asyncio.gather(*futures))
    return graph
...
async def _add_exchange_to_multi_digraph(
    graph: nx.MultiDiGraph, exchange, log=True, suppress=None
):
    tasks = [
        _add_market_to_multi_digraph(
            exchange, symbol, graph, log=log, suppress=suppress
        )
...
# ccxt's fetch_ticker
async def _add_market_to_multi_digraph(
    exchange, market_name: str, graph: nx.DiGraph, log=True, suppress=None
):
    if suppress is None:
        raise ValueError(
            "suppress cannot be None. Must be a list with possible values listed in docstring of"
        ...
        for data_dict in bunch[2]:
            ...
            if not bunch[3]:
                result_graph.add_edge(
                    bunch[0], bunch[1], -math.log(weight), **data_dict
                )
            else:
                result_graph.add_edge(
                    bunch[0], bunch[1], -math.log(1 / weight), **data_dict
                )
