...
class CollectionBuilder:
    ...
    async def build_collections(
        ...
        """
        tasks = [
            self._add_exchange_to_collections(
                exchange,
                ccxt_errors,
            )
        ...
        if write:
            ...
            with open(
                COLLECTIONS_DIR + "singularly_available_markets.json", "w"
            ) as outfile:
                json.dump(self.singularly_available_markets, outfile)

        return self.collections

    async def _add_exchange_to_collections(
        self,
        exchange_name: str,
        ccxt_errors=False,
    ):
        """Add exchange to collections."""
        ...
        for symbol in exchange.symbols:
            if symbol in self.collections:
                ...
                self.collections[symbol] = [
                    self.singularly_available_markets[symbol],
                    exchange_name,
                ]
...
class SymbolCollectionBuilder(CollectionBuilder):
    """Symbol Collection Builder."""

    def __init__(
        self,
        exchanges: list = None,
        symbols: list = None,
        exclusive_currencies: list = None,
...
class SpecificCollectionBuilder(CollectionBuilder):
    ...
    async def _add_exchange_to_collections(
        self, exchange_name: str, ccxt_errors=True, name=True
    ):
        """Add exchange to collections."""
        if name:
            exchange = getattr(ccxt, exchange_name)()
        ...
        if self._check_exchange_meets_criteria(exchange):
            ...
            for market_name in exchange.symbols:
                if market_name in self.collections:
                    ...
                    self.collections[market_name] = [
                        self.singularly_available_markets[market_name],
                        exchange_name,
                    ]
    ...
    def _check_exchange_meets_criteria(self, exchange):
        """Check Exchange Meets Criteria."""
        for key, desired_value in self.rules.items():
            try:
                actual_value = getattr(exchange, key)
            except AttributeError:
                raise ValueError(
                    "{} is not a valid property of {}".format(key, exchange.name)
                )
            ...
            if isinstance(actual_value, list):
                ...
                if isinstance(desired_value, list):
                    for element in desired_value:
                        if not self._element_of_type_in_list(
                            element, type_of_actual_value, actual_value, key
                        ):
                            return False
                else:
                    return self._element_of_type_in_list(
                        desired_value, type_of_actual_value, actual_value, key
                    )
                ...
                if not isinstance(desired_value, dict):
                    raise ValueError(
                        "Exchange attribute {} is a dict but supplied preferred value {} is not a dict".format(
                            key, desired_value
                        )
                    )
                ...
                if isinstance(desired_value, list):
                    if len(desired_value) == 1 and (
                        self.blacklist != (actual_value != desired_value[0])
                    ):
                        return False
    ...
    def _element_of_type_in_list(self, element, actual_value_type, actual_value, key):
        ...
        if not isinstance(element, actual_value_type):
            raise ValueError(
                "Exchange attribute {} is a list of {}s. "
                "A non-{} object was passed.".format(
                    key, str(actual_value_type), str(actual_value_type)
                )
            )
        return self.blacklist == (element not in actual_value)


async def build_specific_collections(
    blacklist=False, write=False, ccxt_errors=False, **kwargs
):
