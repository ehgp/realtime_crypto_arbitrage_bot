...
def clean_request_timeout_for_market(market_name):
    """Clean request timeout for market."""
...
def clean_request_timeout_for_exchanges(exchange_list):
    """Clean request timeout for exchanges."""
    detector = OutlierDetector()
    futures = [
        asyncio.ensure_future(detector.load_markets_for_exchange(exchange_name))
...
def reject_outliers(data: dict, m=2):
    ...
    filtered = [
        exchange_name
        for exchange_name, request_time in data.items()
        if (request_time < mean + m * std)
    ]
    return filtered
