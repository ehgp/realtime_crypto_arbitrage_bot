...
exchangesData = {
    "kucoin": {
        "apiKey": os.environ["KUCOIN_YOUR_API_KEY"],
        "secret": os.environ["KUCOIN_YOUR_SECRET"],
        "transactionFee": 0.01,
    }
}
...
def main():
    ...
    for symbol in symbols:
        print("-----------------------------")
        print(
            "Searching for the best opportunity for {0} on {1}".format(
                symbol, exchanges
            )
        )
        ...
        (
            ...
            bid_exchange_id,
            bid_price,
        ) = get_biggest_spread_by_symbol(exchanges, symbol)
        increase_percentage = (bid_price - ask_price) / ask_price * 100

        print(
            "[{0} - {1}] - [{2}] - Price Spread: {3:.2}%".format(
                ask_exchange_id, bid_exchange_id, symbol, increase_percentage
            )
        )
    ...
    if max_increase_percentage < 0:
        ...
        print("\n--------Best Spread----------")
        print(
            "[{0} - {1}] - [{2}]: Spread percentage: {3:.2}%".format(
                min_ask_exchange_id,
                max_bid_exchange_id,
                exchange_symbol,
        ...
        print(
            "[{0}] - Trading Fee: {1}% = {2:.4} {3}".format(
                min_ask_exchange_id,
                exchangesData[min_ask_exchange_id]["transactionFee"] * 100,
                min_ask_fee,
                exchange_symbol.split("/")[0],
            )
        )
        ...
        print(
            "[{0}] - Trading Fee: {1}% = {2:.4} {3}".format(
                max_bid_exchange_id,
                exchangesData[max_bid_exchange_id]["transactionFee"] * 100,
                max_bid_fee,
                exchange_symbol.split("/")[0],
            )
        )
        ...
        print(
            "You will have to spend: {0:.4} {1}".format(
        ...
        print(
            "You will make {0:.4} {1} profit(including fees)".format(
...
def place_buy_sell_order(
    ask_exchange_id, ask_price, bid_exchange_id, bid_price, symbol, amount
):
    ...
    print(
        "Placing a Buy order on {0} for {1} {2} at price: {3}".format(
    ...
    print(
        "Placing a Sell order on {0} for {1} {2} at price: {3}".format(
