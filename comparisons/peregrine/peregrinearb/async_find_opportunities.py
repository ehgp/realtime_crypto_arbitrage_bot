...
class InterExchangeAdapter(logging.LoggerAdapter):
    ...
    def process(self, msg, kwargs):
        """Process."""
        return (
            "Invocation#{} - Market#{} - {}".format(
                self.extra["invocation_id"], self.extra["market"], msg
            ),
            kwargs,
        )


__all__ = ["OpportunityFinder", "get_opportunity_for_market"]
...
class OpportunityFinder:
    ...
    async def _test_bid_and_ask(self, exchange):
        ...
        """
        self.adapter.info(
            "Checking if {} qualifies for the highest bid or lowest ask for {}".format(
        ...
        # try:
        self.adapter.info(
            "Fetching ticker from {} for {}".format(exchange.id, self.market_name)
        )
        ticker = await exchange.fetch_ticker(self.market_name)
        self.adapter.info(
            "Fetched ticker from {} for {}".format(exchange.id, self.market_name)
        )
        ...
        self.adapter.info(
            "Checked if exchange qualifies for the highest bid or lowest ask for market",
    ...
    async def find_min_max(self):
        """Find min max."""
        tasks = [
            self._test_bid_and_ask(exchange_name)
...
class SuperOpportunityFinder:
    ...
    def __init__(
        ...
        self.adapter = FormatForLogAdapter(
            logging.getLogger(
                "peregrinearb.async_find_opportunities.SuperOpportunityFinder"
            )
        )
    ...
    async def get_opportunities(
        ...
        if price_markets is not None:
            ...
            for market in price_markets:
                tasks.append(
                    self._find_opportunity(market, self.collections[market], True)
                )
        ...
        tasks = [
            self._find_opportunity(market_name, exchange_list)
    ...
    async def _find_opportunity(self, market_name, exchange_list, return_prices=False):
        ...
        for e in exchange_list:
            if e in self.rate_limited_exchanges:
                self.adapter.info(
                    "Sleeping asynchronously because exchange was rate limited",
                    exchange=e,
                    sleeptime=0.1,
                )
        ...
        tasks = [
            self._exchange_fetch_order_book(exchange_name, market_name, current_opp_id)
        ...
        for res in asyncio.as_completed(tasks):
            ...
            if order_book is None:
                ...
                if market_name in self.collections:
                    ...
                    return await self._find_opportunity(
                        market_name, self.collections[market_name]
                    )
        ...
        self.adapter.info(
            "Found opportunity", opportunity=current_opp_id, market=market_name
        )
        if return_prices:
            return opportunity, prices
        return opportunity

    async def _exchange_fetch_order_book(
        self, exchange_name, market_name, current_opp_id
    ):
        """Exchange Fetch Order Book.
        ...
        try:
            order_book = await self.exchanges[exchange_name].fetch_order_book(
                market_name
            )
        except ccxt.DDoSProtection:
            self.adapter.warning(
                "Rate limited for inter-exchange opportunity",
                opportunity=current_opp_id,
                exchange=exchange_name,
                market=market_name,
            )
            ...
            self.adapter.warning(
                "Request timeout for inter-exchange opportunity.",
                opportunity=current_opp_id,
                exchange=exchange_name,
                market=market_name,
            )
        ...
        # If the exchange no longer has the specified market
            ...
            self.adapter.warning(
                "Fetching ticker raised an ExchangeError.",
                opportunity=current_opp_id,
                exchange=exchange_name,
                market=market_name,
            )
            self.adapter.info(
                "Removing exchange from market",
                exchange=exchange_name,
                market=market_name,
                opportunity=current_opp_id,
            )
            ...
            self.adapter.warning(
                "Fetching ticker raised an ExchangeNotAvailable error.",
                opportunity=current_opp_id,
                exchange=exchange_name,
                market=market_name,
            )
        ...
        if order_book["bids"] == [] or order_book["asks"] == []:
            self.adapter.debug(
                "No asks or no bids", exchange=exchange_name, market=market_name
            )
            return None, None
...
async def get_opportunity_for_market(
    ticker, exchanges=None, name=True, invocation_id=0
):
    """Get Opportunity for Market."""
    file_logger.info(
        "Invocation#{} - Finding lowest ask and highest bid for {}".format(
            invocation_id, ticker
        )
    )
    finder = OpportunityFinder(ticker, exchanges=exchanges, name=name)
    result = await finder.find_min_max()
    file_logger.info(
        "Invocation#{} - Found lowest ask and highest bid for {}".format(
            invocation_id, ticker
        )
    )
    return result
