...
def format_graph_for_json(graph, raise_errors=True):
    """Supported types for graph are Graph, DiGraph, MultiGraph, and MultiDiGraph.

    graph must be an instance of one of these types, not a class that inherits from one.
    """
    ...
    if graph_type == "":
        if raise_errors:
            raise TypeError(
                "parameter graph is not of the accepted types.graph is of"
